...
const self = {
  ...
  partialReels: reels => {
    ...
    const url = bestQualityVideo.url;
    return {
      shortcode_media: url
    };
  },
...
module.exports = class Insta {
	...
	getReels(shortcode, { useGraphQL = false } = {}){
		return new Promise((resolve, reject) => {
			if(useGraphQL){
				this._getQueryHashs().then(queryHashs => {
					self.graphQL({ shortcode }, queryHashs.anyPost, this.sessionId)
						.then(data => resolve(self.partialReels(data['shortcode_media'])))
						.catch(reject);
				});
			}
			else {
				self.get(`reel/${shortcode}`, this.sessionId)
					.then(post => resolve(self.partialReels(post)))
					.catch(reject);
			}
		});
	}
